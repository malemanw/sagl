...
<h4><b>Administración de Clases por Docente</b></h4>
</div>
<?php
        global $funciones;
...
<div class="row">
  <div class="col s12 l6">
<blockquote><h5  style="font-size: large;"><b>Seleccione al docente</b></h5></blockquote>  
<div class="input-field">
<select id="seleccionaño" name="seleccionaño" style="display: none;">
<option value="" disabled selected>Seleccione una Opción</option>
	<?php
			$strsql = "SELECT `idusuario`, `usuario` FROM `usuarios` WHERE maestro=1";
			if($stmt = $mysqli->prepare($strsql)){
				$stmt->execute();
				if($stmt->errno == 0){
					$stmt->store_result();
					if($stmt->num_rows > 0){
						...
						while($stmt->fetch()){
						?>
						   <option value="<?php echo $idaño ?>"><?php echo $idaño." - ".$descripcion ?></option>
  				<?php
						}
						?>
</select>
...
<option value="" disabled>Aún no ha seleccionado un docente..</option>
</select>
</div>
  ...
  <div class="col s12 l5">
<blockquote><h5  style="font-size: large;"><b>Clases agregadas al docente</b></h5></blockquote> 	
...
swal({
			title:"¿Continuar?",
			text: "Al continuar se agregarán las clases seleccionadas al docente actual",
			type: "info",
