...
$tmpfontsize=12;
$fontsize = 12;
$pdf->Cell(190, 8, 'Cuadro No. 2', 0,0,'C');
$pdf->Ln();
...
$pdf->Ln();
//query
$strsql = "SELECT clase_curso_detalles.idclase_curso, cursos.nombre_curso, clases.nombre, clases.semestre FROM `clase_curso_detalles` INNER JOIN cursos ON cursos.idcurso=clase_curso_detalles.idcurso INNER JOIN clases ON clases.idclase=clase_curso_detalles.idclase INNER JOIN alumnos_matriculados ON alumnos_matriculados.idcurso=clase_curso_detalles.idcurso WHERE alumnos_matriculados.idexpediente=? and alumnos_matriculados.idaño=? and clase_curso_detalles.idaño=? and clase_curso_detalles.idcurso=? and (clases.semestre=0 OR clases.semestre=?) ORDER BY clases.semestre";
if($st = $mysqli->prepare($strsql)){
    $st->bind_param("siiii", $idexpediente, $idaño, $idaño, $idcurso, $nivel);
	$st->execute();
	if($st->errno == 0){
		$st->store_result();
		if($st->num_rows > 0){
...
if($stmt = $mysqli->prepare($strsql2)){
	...
	if($stmt->errno == 0){
		...
		$total_clase=0;
		if($stmt->num_rows > 0){
			$stmt->bind_result($total, $recuperacion, $parcial);
			while($stmt->fetch()){
		...
		echo "Error al preparar consulta: " . $mysqli->error;
			...
			if($total_clase!=0){
				...
				$pdf->Cell(35, 8, ($total_clase!=0) ? round($promedio= $total_clase/$total_arreglo,2)."%" : $promedio=$recuperacion, 1,0,'C');
				$recuperacion!=0 ? $promedio=$recuperacion : $promedio;
				$promedio_final = $promedio_final + $promedio;
				$corre++;
				$pdf->Cell(0, 8,$recuperacion!=0 ? $recuperacion : "", 1,0,'C');
			}else{
				$pdf->Cell(35, 8,"", 1);
				$pdf->Cell(35, 8,"", 1);
				$pdf->Cell(0, 8,"", 1);
	...
	$pdf->Cell(131, 8, "Promedio Final:", 1,0,'C');
	$pdf->SetFillColor(42, 142, 81);
	$pdf->SetTextColor(255,255,255);
	$pdf->Cell(35, 8,round($promedio_final/$corre,2)."%", 1,0,'C', true);
	$pdf->Cell(0, 8, "", 1,0,'L');
	$pdf->Ln();
	$pdf->SetTextColor(0,0,0);
////
