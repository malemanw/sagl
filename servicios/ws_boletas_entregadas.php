...
switch ($accion)
	{
case "selectanio":
	$strsql = "SELECT alumnos_matriculados.idexpediente, alumnos.nombre_completo, cursos.nombre_curso, anios_militares.descripcion, if(alumnos_matriculados.boleta_entregada =1,'ENTREGADA','NO ENTREGADA') as al_dia
...
if (!empty($_POST["idexpediente"])) {   
	...
	$strsql = "UPDATE `alumnos_matriculados` SET `boleta_entregada`=1 WHERE idexpediente=? and idaño=?";
	if ($stmt = $mysqli->prepare($strsql)) {
		...
		if ($stmt->errno == 0) {
			...
			$idhistorial = 2;
			$obs = "ENTREGADA";
			$strsql = "INSERT INTO `historial_detalles`(`idhistorial`, `idexpediente`, `idusuario`, `obs`) VALUES (?,?,?,?)";
			if($stmt = $mysqli->prepare($strsql)){
				$stmt->bind_param("isss", $idhistorial, $idexpediente, $idusuario, $obs);
				$stmt->execute();
				if($stmt->errno == 0){
					...
					$text = "No se pudo ejecutar la consulta: historial". $stmt->error;
				}
			}
			else{
				$text = "No se puedo preparar la consulta: historial". $mysqli->error;
			}
			////////
		} else {
		...
		echo json_encode($jsonreturn);
			...
			case "reiniciar":
				...
				if (!empty($_POST["año_escolar"])) {   
					$año = $_POST['año_escolar'];   
					$strsql = "UPDATE `alumnos_matriculados` SET `boleta_entregada`=0 WHERE idaño=?";
					if ($stmt = $mysqli->prepare($strsql)) {
